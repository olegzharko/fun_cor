...
static void		fun_ldi_wright_to_reg(t_map *map, int res)
{
	int     i;
    int     num_reg;
...
void	fun_ldi(t_map *map)
{
	...
	step = 0;
	ready_arg(map, iter);
	step = (map->ready_arg[0][0] + map->ready_arg[1][0]) % IDX_MOD + map->pc;
	num = 0;
	r = 0;
	while (r < 4)
	{
